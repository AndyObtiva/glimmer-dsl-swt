module Glimmer
  module SWT
    ...
    class ImageProxy
      ...
      attr_reader :file_path, :jar_file_path, :image_args, :image_data, :swt_image, :aspect_ratio, :auto_resize
      ...
      def initialize(*args)
        ...
        if @file_path
          ...
          @image_args = [DisplayProxy.instance.swt_display, @image_data]
          @swt_image = Image.new(*@image_args)
        else
          @options[:swt_image] = args.first if args.first.is_a?(Image)        
          @image_args = args if @options[:swt_image].nil?          
          @swt_image = @options[:swt_image] || Image.new(*@image_args)
